...
export function TopNav({
  ...
  // Pages where the sidebar (or page chrome) already carries the brand,
  // the page title, and the secondary nav — strip the redundant left
  // content (Pebble. pill + projectName) AND the duplicate Templates +
  // Help icons from the right side. AuthMenu + ThemeToggle remain so
  // users can still log out / switch theme.
  // Marc 2026-05-25: extended from {dashboard, community} to ALL signed-in
  // workspace surfaces after Chrome MCP audit found too much chrome
  // clutter on /templates, /inbox, /settings, /integrations.
  const COMPACT_PATHS = [
    "/dashboard",
    "/community",
    "/templates",
  ...
  const compact = COMPACT_PATHS.some((p) => pathname === p || pathname.startsWith(p + "/"));
