# ink_studio_steel — Pebble Engine template

Cold industrial brushed-steel Next.js 14 (App Router) template derived from
the `tattoo_studio` DNA. Where the original is cold-gothic gold-glow and the
oxblood variant is warm-cathedral, this is the cold industrial workshop
counterpart — deep cool-charcoal, brushed steel grey, muted aged-brass
spark. Placeholder brand: **Vault Iron Studio**.
...
- This template shares the design language but uses an **original, generic
  brand** so it can be instantiated for any tattoo / barbershop / leather
  workshop / speakeasy / cigar lounge / artisan business that wants the
  pitch-black + gold + blackletter aesthetic.
...
- Deep cool-charcoal `#0E1014` background, brushed-steel `#A8B0BD` primary
  accent, muted aged-brass `#C0B47A` highlight spark
- Gold-glow text-shadow on all hero / booking headlines (stacked
  text-shadow at 20px + 40px)
...
4. **Services** — three-column "what we do" block (custom / flash /
