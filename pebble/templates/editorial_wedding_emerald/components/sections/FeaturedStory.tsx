...
export function FeaturedStory() {
  ...
  return (
    <section id="featured" ref={targetRef} className="relative" style={{ height: `${slideCount * 100}vh` }}>
      <div className="sticky top-0 h-screen overflow-hidden bg-[#0a2820]">
        <motion.div
            ...
            <div key={i} className="h-full flex-shrink-0 flex items-center justify-center px-[10vw] py-16" style={{ width: `${100 / slideCount}%` }}>
              {slide.kind === "intro" && (
                <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-12 items-center">
                  ...
                  <div>
                    <p className="text-[#c9a96e] text-sm tracking-widest uppercase mb-2">{slide.label}</p>
                    <h2 className="font-[family-name:var(--font-display)] italic text-4xl md:text-5xl mb-4 text-[#f5f0dc]">
                      {slide.couple}
                    </h2>
                    <p className="text-[#dcb780] font-[family-name:var(--font-display)] italic text-xl mb-6">
                      {slide.venue}
                    </p>
                    <p className="text-[#f5f0dc]/70 leading-relaxed">&ldquo;{slide.quote}&rdquo;</p>
                  </div>
                </div>
              )}
              {slide.kind === "photo" && (
              ...
              {slide.kind === "outro" && (
                <div className="text-center">
                  <h3 className="font-[family-name:var(--font-display)] italic text-3xl text-[#f5f0dc] mb-4">
                    {slide.quote}
                  </h3>
                  <p className="text-[#dcb780]">{slide.subline}</p>
                </div>
              )}
            </div>
          ))}
