...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#0F1115]/80 backdrop-blur-md border-b border-white/5 py-5 px-6 sm:px-12 md:px-20 flex items-center justify-between">
      ...
      {open && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#0F1115] border-b border-white/5 px-6 py-6 flex flex-col gap-5 z-50">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
