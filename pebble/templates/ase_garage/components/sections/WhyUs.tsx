        ...
        <div className="grid md:grid-cols-4 gap-6">
          {TRUST_POINTS.map((tp, i) => (
            <Reveal key={tp.title} delay={0.05 * i}>
              <div className="p-6 bg-[#fafaf9] border-b-4 border-[#1e293b] shadow-sm h-full">
                <p className="text-[#facc15] font-[family-name:var(--font-display)] text-4xl mb-2">
                  {tp.number}
                </p>
                <h3 className="font-bold text-lg mb-2">{tp.title}</h3>
                <p className="text-sm text-[#1e293b]/70 leading-relaxed">{tp.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
