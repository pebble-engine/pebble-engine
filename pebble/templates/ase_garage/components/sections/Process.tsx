          ...
          {PROCESS_STEPS.map((step, i) => (
            <Reveal key={step.number} delay={0.1 * i}>
              <div className="relative text-center">
                <div className="w-12 h-12 bg-[#facc15] text-[#1e293b] font-[family-name:var(--font-display)] text-xl rounded-full flex items-center justify-center mx-auto mb-4">
                  {step.number}
                </div>
                <h3 className="font-bold text-xl mb-2 uppercase">{step.title}</h3>
                <p className="text-sm text-[#1e293b]/70 px-4">{step.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
