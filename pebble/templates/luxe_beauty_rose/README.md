# luxe_beauty_rose — Pebble Engine template

Editorial luxury-beauty Next.js 14 (App Router) template derived from the
`beauty_ethereal` DNA — **warm rose variant**. Placeholder brand:
**Rose & Veil Apothecary**.

Sister template of `luxe_beauty` (ethereal mint). Same structure,
components, and content shape — swapped palette (warm rose / blush /
lavender on a blush-cream surface, deep berry text) and a softer,
more inviting tonal voice. Still premium, still editorial; less austere.
...
- Material Design 3 surface tokens — light-mode native (rose/blush)
...
1. **Hero** — circular logo orb on warm rose cloud bg, italic Bodoni
   headline, Pinyon Script cursive accent, dual rounded-full CTAs
...
- **Sky video as fixed background layer** — replaced with a soft rose
  cloud gradient (`ethereal-bg` utility) + low-opacity hero photo.
  Cheaper, no asset dependency.
