...
# pebble.text / pebble.next_config_patch are imported LAZILY inside
# run_instantiate_template — they're heavy and only matter for the POST
# path. Deferring them keeps GET /api/templates working even if those
# modules are absent on a thin branch checkout.
...
def run_instantiate_template(handler) -> None:
    ...
    # Lazy import — only the POST path needs the sanitizer
    ...
    try:
        # Lazy import — only the POST path needs the patcher
