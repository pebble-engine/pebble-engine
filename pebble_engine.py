...
| **Motion** | Defer to the Design DNA block's motion intensity. (Cinematic, subtle, or aggressive — the DNA decides. Never use the paid `gsap/SplitText` plugin in full Next.js builds; the `AnimatedHeading` foundation component handles per-character entrance instead.) |
| **GSAP easing** | `{easing}` |
...
# --------------------------------------------------------------------------
#
# Implementation moved to pebble/llm_parse.py (2026-06-01). Re-exported here
# so existing `pebble_engine.parse_files` / `parse_deletions` /
# `detect_truncation` imports keep working unchanged — tests, the repair
# loop's lazy-import shim, and the server modules that hoisted these symbols
# via `pe = _engine(); parse_files = pe.parse_files`. New callers should
# import directly from `pebble.llm_parse`.

from pebble.llm_parse import (  # noqa: E402
    FILE_OPEN_RE,
    detect_truncation,
    parse_files,
    parse_deletions,
...
class PebbleHandler(BaseHTTPRequestHandler):
    ...
    def _handle_health(self):
        from pebble.server.health import run_health
        run_health(self)
    ...
    def _handle_preview(self):
        from pebble.server.preview_serve import run_preview
        run_preview(self)
