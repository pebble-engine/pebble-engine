...
# ---------- setup_call (one-time payment) ----------------------------------

@pytest.fixture
def setup_call_env(monkeypatch):
    """Env with all three price IDs set."""
    monkeypatch.setenv("STRIPE_SECRET_KEY", "sk_test_unit-test-key")
    monkeypatch.setenv("PEBBLE_STRIPE_STARTER_PRICE_ID", "price_starter_unit")
    monkeypatch.setenv("PEBBLE_STRIPE_PRO_PRICE_ID", "price_pro_unit")
    monkeypatch.setenv("PEBBLE_STRIPE_SETUP_PRICE_ID", "price_setup_unit")
    monkeypatch.setenv("PEBBLE_PUBLIC_URL", "https://app.test.pebble")


def test_setup_call_uses_payment_mode(signed_in_user, setup_call_env, fake_stripe, monkeypatch):
    """plan=setup_call → Stripe session must use mode='payment', not
    'subscription' — prevents the caller minting a free subscription."""
    monkeypatch.delenv("PEBBLE_SETUP_CALL_LINK", raising=False)
    from pebble.server import stripe_checkout

    h = FakeHandler({"plan": "setup_call"})
    stripe_checkout.run_create_session(h)

    assert h.status == 200
    kwargs = fake_stripe.checkout.Session.create.call_args.kwargs
    assert kwargs["mode"] == "payment"
    assert kwargs["line_items"] == [{"price": "price_setup_unit", "quantity": 1}]


def test_setup_call_redirects_to_calendar_link_on_success(
    signed_in_user, setup_call_env, fake_stripe, monkeypatch,
):
    """When PEBBLE_SETUP_CALL_LINK is set, success_url IS the calendar link
    so the user lands on the booking page immediately after payment."""
    ...
    kwargs = fake_stripe.checkout.Session.create.call_args.kwargs
    assert kwargs["success_url"] == "https://calendly.com/pebble/setup"


def test_setup_call_falls_back_to_success_page_without_calendar_link(
    signed_in_user, setup_call_env, fake_stripe, monkeypatch,
):
    """When PEBBLE_SETUP_CALL_LINK is NOT set, redirect to the standard
    billing success page instead of leaving the user stranded."""
    monkeypatch.delenv("PEBBLE_SETUP_CALL_LINK", raising=False)
    from pebble.server import stripe_checkout
    ...
    kwargs = fake_stripe.checkout.Session.create.call_args.kwargs
    assert kwargs["success_url"].startswith("https://app.test.pebble/billing/success")


def test_setup_call_has_no_subscription_data(
    signed_in_user, setup_call_env, fake_stripe, monkeypatch,
):
    """One-time payment must NOT send subscription_data — if Stripe
    receives that for a payment-mode session it rejects the call."""
    monkeypatch.delenv("PEBBLE_SETUP_CALL_LINK", raising=False)
    from pebble.server import stripe_checkout
...
def test_returns_503_when_setup_price_id_missing(
    signed_in_user, fake_stripe, monkeypatch,
):
    """Bootstrap not run for setup_call → price-ID env var missing → 503."""
    from pebble.server import stripe_checkout

    monkeypatch.setenv("STRIPE_SECRET_KEY", "sk_test_x")
    monkeypatch.delenv("PEBBLE_STRIPE_SETUP_PRICE_ID", raising=False)
    monkeypatch.setenv("PEBBLE_PUBLIC_URL", "https://app.test.pebble")
    h = FakeHandler({"plan": "setup_call"})
    stripe_checkout.run_create_session(h)
    assert h.status == 503
