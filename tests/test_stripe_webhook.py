...
# ---------- checkout.session.completed (setup_call) -----------------------

def _checkout_event(
    *,
    pebble_plan: str = "setup_call",
    payment_status: str = "paid",
    customer_email: str | None = "buyer@example.com",
    pebble_user_id: str | None = "uuid-marc-abc",
) -> dict:
    """Build a checkout.session.completed event dict."""
    metadata: dict = {}
    if pebble_plan:
        metadata["pebble_plan"] = pebble_plan
    if pebble_user_id:
        metadata["pebble_user_id"] = pebble_user_id
    obj: dict = {
        "id": "cs_test_session",
...
def test_setup_call_checkout_sends_confirmation_email_with_link(
    with_secret, output_root, verified_event, monkeypatch,
):
    """A paid setup_call checkout → confirmation email with the calendar link
    is sent via get_sender(). The subscription sentinel is NOT touched."""
    from pebble.server import stripe_webhook

    monkeypatch.setenv("PEBBLE_SETUP_CALL_LINK", "https://calendly.com/pebble/setup")
    ...
    class FakeSender:
        def send(self, msg):
            ...
            return {"ok": True, "provider": "fake", "id": "fake_id"}

    monkeypatch.setattr(
        "pebble.server.stripe_webhook.get_sender",
        lambda: FakeSender(),
    )

    h = verified_event(_checkout_event())
    stripe_webhook.run_stripe_webhook(h)

    assert h.status == 200
    assert h.body.get("action") == "applied"
    assert len(sent_messages) == 1
    msg = sent_messages[0]
    assert msg.to == "buyer@example.com"
    assert "Setup Call" in msg.subject
    assert "https://calendly.com/pebble/setup" in msg.text
    # No sentinel written — this is a one-time payment, not a subscription
    assert list(output_root.glob(".users/*/subscription.json")) == []


def test_setup_call_checkout_sends_fallback_email_when_no_calendar_link(
    with_secret, output_root, verified_event, monkeypatch,
):
    """When PEBBLE_SETUP_CALL_LINK is not set, still sends a confirmation
    email (no calendar link, just 'we'll be in touch')."""
    from pebble.server import stripe_webhook

    monkeypatch.delenv("PEBBLE_SETUP_CALL_LINK", raising=False)
    ...
    monkeypatch.setattr("pebble.server.stripe_webhook.get_sender", lambda: FakeSender())
...
def test_setup_call_checkout_skips_when_not_paid(
    with_secret, output_root, verified_event, monkeypatch,
):
    """Async payment method (e.g. bank transfer) fires checkout.session.completed
    with payment_status='unpaid'. We must NOT send the confirmation yet —
    wait for the 'paid' event instead."""
    from pebble.server import stripe_webhook

    monkeypatch.setenv("PEBBLE_SETUP_CALL_LINK", "https://calendly.com/pebble/setup")
    monkeypatch.setattr("pebble.server.stripe_webhook.get_sender", lambda: None)

    h = verified_event(_checkout_event(payment_status="unpaid"))
    stripe_webhook.run_stripe_webhook(h)

    assert h.status == 200
    assert h.body.get("action") == "skipped"


def test_setup_call_checkout_ignored_when_not_setup_call(
    with_secret, output_root, verified_event, monkeypatch,
):
    """checkout.session.completed for a subscription checkout (plan=starter)
    must be ignored — subscription events handle those state changes."""
    from pebble.server import stripe_webhook

    monkeypatch.setattr("pebble.server.stripe_webhook.get_sender", lambda: None)

    h = verified_event(_checkout_event(pebble_plan="starter"))
    stripe_webhook.run_stripe_webhook(h)

    assert h.status == 200
    assert h.body.get("action") == "ignored"


def test_setup_call_email_failure_is_nonfatal(
    with_secret, output_root, verified_event, monkeypatch,
):
    """Email send failure must NOT bubble up as a 500. The $99 is already
    charged — raising here would leave Stripe without an ACK and cause
    retries, potentially sending multiple broken emails."""
    from pebble.server import stripe_webhook
    ...
    monkeypatch.setattr("pebble.server.stripe_webhook.get_sender", lambda: BrokenSender())

    h = verified_event(_checkout_event())
    stripe_webhook.run_stripe_webhook(h)

    assert h.status == 200  # not 500
    assert h.body.get("action") == "applied"


def test_setup_call_email_address_not_logged(
    with_secret, output_root, verified_event, monkeypatch, caplog,
):
    """PII regression — customer_email must NOT appear verbatim in logs."""
    import logging
    from pebble.server import stripe_webhook

    monkeypatch.setenv("PEBBLE_SETUP_CALL_LINK", "https://calendly.com/pebble/setup")
    monkeypatch.setattr("pebble.server.stripe_webhook.get_sender",
                        lambda: type("S", (), {"send": lambda s, m: {"ok": True, "provider": "fake", "id": "x"}})())

    caplog.set_level(logging.INFO, logger="pebble")
    h = verified_event(_checkout_event(customer_email="buyer@example.com"))
    stripe_webhook.run_stripe_webhook(h)
