...
def test_favicon_defined_fails_when_none_defined(good_build):
    ...
    assert result.status == "fail"
    assert "favicon" in result.message.lower()


def test_favicon_defined_skips_when_no_site_dir(tmp_path):
    empty = tmp_path / "no-site"
    empty.mkdir()
    ctx = BuildContext.load(empty)
    assert checks.favicon_defined(ctx).status == "skip"
...
def test_env_example_passes_on_good_build(good_build):
    """good_build includes .env.example with RESEND_API_KEY — check passes."""
    ctx = BuildContext.load(good_build)
    assert checks.env_example_present(ctx).status == "pass"


def test_env_example_fails_when_file_missing(good_build):
    (good_build / "site" / ".env.example").unlink()
    ctx = BuildContext.load(good_build)
    result = checks.env_example_present(ctx)
    assert result.status == "fail"
    assert ".env.example" in result.message


def test_env_example_fails_when_resend_key_absent(good_build):
    """File exists but doesn't mention RESEND_API_KEY → fail."""
    (good_build / "site" / ".env.example").write_text("NEXT_PUBLIC_SITE_URL=https://example.com\n")
    ctx = BuildContext.load(good_build)
    result = checks.env_example_present(ctx)
    assert result.status == "fail"
...
def test_env_example_skips_when_no_site_dir(tmp_path):
    empty = tmp_path / "no-site"
    empty.mkdir()
    ctx = BuildContext.load(empty)
    assert checks.env_example_present(ctx).status == "skip"
